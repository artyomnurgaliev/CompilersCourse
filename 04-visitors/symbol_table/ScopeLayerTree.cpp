...
void ScopeLayerTree::AddMapping(Symbol name, ScopeLayer *layer) {
  if (layer_mapping_.find(name) != layer_mapping_.end()) {
    throw std::runtime_error("Symbol has already been declared");
  }
  layer_mapping_[name] = layer;
}

ScopeLayer *ScopeLayerTree::GetScopeByName(Symbol name) {
  if (layer_mapping_.find(name) == layer_mapping_.end()) {
...
std::shared_ptr<ObjectType> ScopeLayerTree::Get(Symbol symbol) {
